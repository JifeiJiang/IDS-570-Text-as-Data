...
# In step 9, I re-run BERT cluster and interpret of the "English Clusters?" and "English Clusters?" separatedly
...
# 2. Extracting data from the "English cluster?"
english_idx = np.where(old_labels == "English Cluster?")[0]
X_english = X[english_idx]
metadata_english = [metadata[i] for i in english_idx]

print(f"Filtered {len(X_english)} English occurrences from the main dataset.\n")

# 3. Re-clustering queer in that cluster
# 3.1 Find optimal k using silhouette score
k_range = range(2, min(8, len(X_english) // 2))
sil_scores = []

for k in k_range:
    km = KMeans(n_clusters=k, random_state=42, n_init=10)
    labels = km.fit_predict(X_english)
    sil_scores.append(silhouette_score(X_english, labels))
...
new_EN_labels = km_queer.fit_predict(X_english)

# 3.3 Map cluster numbers to custom context names
label_map = {0: "Acadmeic Theory", 1: "Popular Culture"}
mapped_EN_labels = [label_map[c] for c in new_EN_labels]

print(f"\nNew English Queer clusters (k={K_QUEER}):")
for c in range(K_QUEER):
    count = (new_EN_labels == c).sum()
    print(f"  {label_map[c]}: {count} occurrences")

# 3.4 PCA visualization
pca_queer = PCA(n_components=2)
queer_2d = pca_queer.fit_transform(X_english)
...
ax.set_title("Contextual Embeddings of English Cluster 'Queer' within Text")
ax.legend()
ax.grid(True, alpha=0.2)
plt.tight_layout()
plt.savefig("English_queer_embeddings_pca.png", dpi=150, bbox_inches="tight")
plt.show()

print("\nSaved plots to English_queer_cluster_selection.png and English_queer_embeddings_pca.png")
...
texts = [meta.get("sentence", "") for meta in metadata_english]
labels_array = np.array(mapped_EN_labels)
...
QUEER_SENSES = ["Acadmeic Theory", "Popular Culture"]
...
output = {
    "embeddings": X_english.tolist(),
    "queer_cluster_labels_new": mapped_EN_labels,
    "metadata": metadata_english
}

with open(Path("data") / "EN_embeddings_and_labels.json", "w", encoding="utf-8") as f:
    json.dump(output, f, ensure_ascii=False)

print("Saved internal English sub-clusters to data/EN_embeddings_and_labels..json")