...
#0-2.Clean Data (Because I have tokenized them before, so I redo them here)
text_tbl <- texts_df %>%
  mutate(text_clean = str_replace_all(text, c(
    "ſ" = "s",
    "2dly" = "secondly",
...
anno_df %>% glimpse()
#3. Create an example parsed sentence for 17 and 20 (sentence_id is what I randomly pick)
example_text17 <- anno_df %>%
  filter(document == "text17", sentence_id == 20) %>%
  select(token, token_id, head_token_id, dep_rel)
example_text17 %>%
  knitr::kable(
    caption = "Dependency structure from real text of Text17")
example_text20 <- anno_df %>%
  filter(document == "text20", sentence_id == 21) %>%
  select(token, token_id, head_token_id, dep_rel)
example_text20 %>%
  knitr::kable(
    caption = "Dependency structure from real text of Text20")
...
print(all_measures)
write.csv(all_measures,
          "C:/Users/jifei/Desktop/all_measures.csv",
          row.names = FALSE)
#8. Visualization
